...
public class TextDatabase : MonoBehaviour
{
    ...
    public Dictionary<string, string> Clues = new Dictionary<string, string> {
        ...
        {"Adam Abrams Clue 1", "Evidence #1 \nCase: 342 \nEntrant: Adam Abrams \nDescription: REDACTED.\n"},

        {"Adam Abrams Clue 2", "Evidence #2 \nCase: 342 \nEntrant: Adam Abrams\n" +
    ...
    public Dictionary<string, string> Notes = new Dictionary<string, string> {
        ...
        {"Louis Prejean Note 1", "            My name is Louis Prejean. I’m writing this note because I don’t have much time left. There’s already been one attempt on my life. My secret is out. My enemies know. But I don’t know who my enemies are, so I jump at every shadow.\n" +
    ...
    public string GetMiscItemValue(string key)
    {
        return (GetValByKey(key,MiscItemDescriptions));
    }

    public string GetWeaponValue(string key)
    {
        return (GetValByKey(key,WeaponDescriptions));
    }

    public string GetCharacterValue(string key)
    {
        return (GetValByKey(key,CharacterDescriptions));
    }

    public string GetRoomValue(string key)
    {
        return (GetValByKey(key,RoomDescriptions));
    }

    public string GetClueValue(string key)
    {
        return (GetValByKey(key,Clues));
    }

    public string GetNoteValue(string key)
    {
        return (GetValByKey(key,Notes));
    }

    public string GetClueOrNoteValue(string key)
    {
        string noteVal = GetNoteValue(key);
        string clueVal = GetClueValue(key);
        if (noteVal != "Error: TextDatabase Key has no Value")
        {
            return noteVal;
        }
        else
    ...
    public string GetValByKey( string key, Dictionary<string,string> myDictionary)
    {
        var val = "Error: TextDatabase Key has no Value";
